 ...
 */
"use strict";var isWorking,attempt=_dereq_("attempt-x"),nativeReduce="function"==typeof Array.prototype.reduce&&Array.prototype.reduce;if(nativeReduce){var res;if((isWorking=attempt.call([],nativeReduce,function(acc){return acc}).threw)&&(isWorking=!1===(res=attempt.call(Object("abc"),nativeReduce,function(acc,c){return acc+c},"x")).threw&&"xabc"===res.value),isWorking&&(isWorking=!1===(res=attempt.call(function(){return arguments}(1,2,3),nativeReduce,function(acc,arg){return acc+arg},1)).threw&&7===res.value),isWorking&&(isWorking=!1===(res=attempt.call({0:1,1:2,3:3,4:4,length:4},nativeReduce,function(acc,arg){return acc+arg},2)).threw&&8===res.value),isWorking){var doc="undefined"!=typeof document&&document;if(doc){var fragment=doc.createDocumentFragment(),div=doc.createElement("div");fragment.appendChild(div),isWorking=!1===(res=attempt.call(fragment.childNodes,nativeReduce,function(acc,node){return acc[acc.length]=node,acc},[])).threw&&1===res.value.length&&res.value[0]===div}}isWorking&&(isWorking=!1===(res=attempt.call("ab",nativeReduce,function(_,__,___,list){return list})).threw&&"object"==typeof res.value)}var $reduce;if(nativeReduce&&isWorking)$reduce=function reduce(array,callBack){var args=[callBack];return arguments.length>2&&(args[1]=arguments[2]),nativeReduce.apply(array,args)};else{var boxedString=Object("a"),splitString="a"!==boxedString[0]||0 in boxedString==!1,strSplit=splitString&&String.prototype.split,isString=splitString&&_dereq_("is-string"),toLength=_dereq_("to-length-x"),toObject=_dereq_("to-object-x"),assertIsFunction=_dereq_("assert-is-function-x");$reduce=function reduce(array,callBack){var object=toObject(array);assertIsFunction(callBack);var iterable=splitString&&isString(object)?strSplit.call(object,""):object,length=toLength(iterable.length),argsLength=arguments.length;if(0===length&&argsLength<3)throw new TypeError("reduce of empty array with no initial value");var result,i=0;if(argsLength>2)result=arguments[2];else for(;;){if(i in iterable){result=iterable[i],i+=1;break}if((i+=1)>=length)throw new TypeError("reduce of empty array with no initial value")}for(;i<length;)i in iterable&&(result=callBack(result,iterable[i],i,object)),i+=1;return result}}module.exports=$reduce},{"assert-is-function-x":3,"attempt-x":4,"is-string":14,"to-length-x":24,"to-object-x":26}],2:[function(_dereq_,module,exports){/**
